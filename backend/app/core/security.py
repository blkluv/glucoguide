...
def verify_token(token: str, db: Session, required_scopes: List[str]):
...
async def get_user_token(user: User | Patient | Doctor, message: str, status: int | None = 200, redirect_url: str | None = None):
  ...
  if(redirect_url):
    ...
    response.set_cookie('access_token', access_token, expires=access_token_expires, secure=True)
    response.set_cookie('refresh_token', refresh_token, expires=refresh_token_expires, secure=True)
    return response 
