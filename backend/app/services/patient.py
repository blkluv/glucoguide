...
class PatientService:
  ...
  async def get_patient_information(session_user: Patient, redis: Redis):
    if (cached_profile := redis.get(f"profile_info_{session_user.id}")):  # return patient profile cache if available
      result = json.loads(cached_profile)
      return ResponseHandler.fetch_successful(f'sucessfully retrieved patient profile using caching - {session_user.id}', result)
    ...
    return ResponseHandler.fetch_successful(f'f"successfully retrieved patient profile - {session_user.id}",  ', profile_data)
  ...
  async def change_user_password(updated_data: UserPasswordChange, session_user: Patient, db: Session):
    ...
    if not verify_password(decrypted_old_pass, session_user.password):
      raise HTTPException(status_code=400, detail=f"old password is incorrect - {session_user.id}")
